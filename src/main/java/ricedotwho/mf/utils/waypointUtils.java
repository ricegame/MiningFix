...
public class waypointUtils {
    ...
    public static void renderWaypointText(String str, BlockPos loc, float partialTicks) {
        ...
        double z = loc.getZ() + 0.5 - viewerZ; // loc is the waypoint location
        ...
        drawNametag("("+EnumChatFormatting.WHITE+Math.round(dist)+"m)");
    ...
    // bettermap draw box method
    public static void drawOutlineAtBlock(double x, double y, double z, Color colour, float alpha, double w, double h) {
        GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        GlStateManager.disableBlend();
        GlStateManager.depthMask(false);
        GlStateManager.pushMatrix();
        GL11.glLineWidth(3);
        float colorR = colour.getRed() / 255f;
        float colorG = colour.getGreen() / 255f;
        float colorB = colour.getBlue() / 255f;

        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldRenderer = tessellator.getWorldRenderer();
        worldRenderer.begin(GL11.GL_LINE_STRIP, DefaultVertexFormats.POSITION_COLOR);
        worldRenderer.pos(x + w, y + h, z + w).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x + w, y + h, z).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x, y + h, z).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x, y + h, z + w).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x + w, y + h, z + w).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x + w, y, z + w).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x + w, y, z).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x, y, z).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x, y, z + w).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x, y, z).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x, y + h, z).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x, y, z).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x + w, y, z).color(colorR, colorG, colorB, alpha).endVertex();
    ...
    // Inspired by ColeWeight
    public static void drawLineToBlock(double px, double py, double pz,double x, double y, double z, Color colour, float alpha) {
        GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        GlStateManager.disableBlend();
        GlStateManager.depthMask(false);
        GlStateManager.pushMatrix();
        ...
        worldRenderer.pos(px, py, pz).color(colorR, colorG, colorB, alpha).endVertex();
        worldRenderer.pos(x + 0.5, y, z + 0.5).color(colorR, colorG, colorB, alpha).endVertex();
        tessellator.draw();
